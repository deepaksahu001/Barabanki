...
var map,blockLyr,mkrLyr,scLyr=null,extFacLyr=null;
var blblMks=[],mkrMap={DH:[],CHC:[],ACHC:[],PHC:[]};
var mkrVis={DH:true,CHC:true,ACHC:true,PHC:true};
var CURBLK=null,fillOn=true,fillOp=0.18,CM='default',PL='landscape';
...
window.onload=function(){
  ...
  curTile=tileLyrs.osm;curTile.addTo(map);
  map.on('zoomend moveend',function(){if(el('SB').classList.contains('on'))updSB();});
  map.on('click',function(){if(CURBLK){resetBStyle();CURBLK=null;showPH();}});
  mkrLyr=L.layerGroup().addTo(map);
  ...
  el('lstep').textContent='Adding markers...';
  buildMkrs();buildBlbl();buildBTogList();buildBClrList();buildLegend();
  try{var b=blockLyr.getBounds();if(b.isValid())map.fitBounds(b,{padding:[8,8]});}catch(e){}
  setTimeout(function(){
    map.invalidateSize();
    el('loadmask').classList.add('done');
    // Auto-load the facility sheet
    loadFac();
  },400);
...
function getBC(nm){
  ...
  if(CM==='live'){var d=LIVE[nm];if(!d)return'#e0e0e0';var v=d.pregnant||0;return v>=20?'#e53935':v>=10?'#ff7043':v>=5?'#ffa726':'#aed581';}
  return BCOLORS[nm]||'#ccc';
}
function applyBS(){blockLyr.setStyle(function(f){return bStyle(f.properties.tehs_name);});}
function resetBStyle(){applyBS();}
...
function setCM(m){CM=m;el('mc-sec').style.display=(m==='manual')?'block':'none';applyBS();updChoLeg(m);}
...
function updChoLeg(m){
  ...
  else if(m==='live'){ti.textContent='Pregnant Women';it.innerHTML='<div class="cho-row"><div class="cho-sw" style="background:#e53935"></div>20+</div><div class="cho-row"><div class="cho-sw" style="background:#ff7043"></div>10-19</div><div class="cho-row"><div class="cho-sw" style="background:#ffa726"></div>5-9</div><div class="cho-row"><div class="cho-sw" style="background:#aed581"></div>&lt;5</div><div class="cho-row"><div class="cho-sw" style="background:#e0e0e0"></div>No data</div>';}
}

// Block selection
function selBlock(nm,lyr){
  CURBLK=nm;applyBS();
  if(lyr)lyr.setStyle({weight:3,color:'#e53935',fillOpacity:Math.max(fillOp,0.35),fillColor:getBC(nm)});
  goTab('info',null);showBInfo(nm);
  if(window.innerWidth<=768){openSB();}
...
function buildMkrs(){
  mkrLyr.clearLayers();mkrMap={DH:[],CHC:[],ACHC:[],PHC:[]};
...
function togFLyr(type,on){
  mkrVis[type]=on;
  if(mkrMap[type])mkrMap[type].forEach(function(mk){if(on){if(!mkrLyr.hasLayer(mk))mkrLyr.addLayer(mk);}else{if(mkrLyr.hasLayer(mk))mkrLyr.removeLayer(mk);}});
}
function togSCLyr(on){
  if(on){
    if(!scLyr){var mks=SC_DATA.map(function(m){var col=(m.status==='Active')?'#f57c00':'#aaa';var mk=L.circleMarker([m.lat,m.lon],{radius:4,fillColor:col,color:'white',weight:1,fillOpacity:0.85});mk.bindTooltip('<b>'+m.name+'</b><br>SC &middot; '+m.status+'<br>'+m.block,{direction:'top',offset:[0,-6]});return mk;});scLyr=L.layerGroup(mks);}
    scLyr.addTo(map);
  } else {if(scLyr&&map.hasLayer(scLyr))map.removeLayer(scLyr);}
}
...
function buildBTogList(){
  var cont=el('block-tog-list');cont.innerHTML='';
...
function togBlock(chk){
  ...
  applyBS();buildMkrs();buildBlbl();buildLegend();
}
function allBlocks(on){
  ...
  if(!on){CURBLK=null;showPH();}applyBS();buildMkrs();buildBlbl();buildLegend();
}

// Block color pickers (Varanasi style)
function buildBClrList(){
  var c=el('bclr-list');c.innerHTML='';
  BNAMES.forEach(function(nm){
    var col=BCOLORS[nm];var r=document.createElement('div');
    r.style.cssText='display:flex;align-items:center;margin-bottom:5px;gap:6px;';
    var dot=document.createElement('div');dot.id='bdot-'+nm.replace(/\s/g,'_');
    dot.style.cssText='width:9px;height:9px;border-radius:50%;background:'+col+';flex-shrink:0;';
    var lbl=document.createElement('span');lbl.style.cssText='font-size:11px;font-weight:600;color:var(--t);flex:1;';lbl.textContent=nm;
    var ci=document.createElement('input');ci.type='color';ci.value=col;ci.className='cpick';
    ci.setAttribute('data-nm',nm);
    ci.addEventListener('input',function(){changeBC(this.getAttribute('data-nm'),this.value);});
    r.appendChild(dot);r.appendChild(lbl);r.appendChild(ci);c.appendChild(r);
  });
  // Manual block colors list
  var cm=el('bclr-manual');if(!cm)return;cm.innerHTML='';
  BNAMES.forEach(function(nm){
    var col=BCOLORS[nm];var r=document.createElement('div');
    r.style.cssText='display:flex;align-items:center;margin-bottom:5px;gap:6px;';
    var dot=document.createElement('div');dot.style.cssText='width:9px;height:9px;border-radius:50%;background:'+col+';flex-shrink:0;';
    var lbl=document.createElement('span');lbl.style.cssText='font-size:11px;font-weight:600;color:var(--t);flex:1;';lbl.textContent=nm;
    var ci=document.createElement('input');ci.type='color';ci.value=col;ci.className='cpick';
    ci.setAttribute('data-nm',nm);ci.addEventListener('input',function(){changeBC(this.getAttribute('data-nm'),this.value);});
    r.appendChild(dot);r.appendChild(lbl);r.appendChild(ci);cm.appendChild(r);
  });
}

// Legend (Varanasi style)
function buildLegend(){
  var bl=el('leg-blocks');bl.innerHTML='<div class="ltitle" style="margin-bottom:4px;">Blocks</div>';
  BNAMES.forEach(function(nm){if(!blockOn[nm])return;var d=document.createElement('div');d.className='litem';d.innerHTML='<div class="lsw" style="background:'+(fillOn?BCOLORS[nm]:'#ccc')+'"></div><span>'+nm+'</span>';bl.appendChild(d);});
  var fl=el('leg-fac');
  var h='<div style="height:1px;background:#eee;margin:5px 0;"></div>';
  [{k:'DH',l:'District Hospital'},{k:'CHC',l:'CHC'},{k:'ACHC',l:'Additional CHC'},{k:'PHC',l:'PHC'},{k:'SC',l:'Sub Centre'}].forEach(function(t){h+='<div class="litem"><div class="ldot2" style="background:'+TC[t.k]+'"></div><span>'+t.l+'</span></div>';});
  fl.innerHTML=h;
}
function togLeg(on){el('leg').classList.toggle('hide',!on);}
...
function parseGviz(resp){
  ...
  var hmap={block:['block','block name','tehsil'],name:['facility name','name','facility_n'],
  ...
  // Fallback to HFR positions if auto-detect fails
  if(IDX.block===-1)IDX.block=8;if(IDX.name===-1)IDX.name=13;
  ...
  (resp.table.rows||[]).forEach(function(row){
    ...
    var obj={name:(''+gvC(row,IDX.name)).trim(),type:type,cls:cls,block:(''+gvC(row,IDX.block)).trim(),lat:lat,lng:lng,status:status};
    FAC_RAW.push(obj);
    var p=prev[type]||{};
    if(!FAC_TYPES[type])FAC_TYPES[type]={color:(p.col||TC[type]||'#555'),icon:TI[type]||'📍',size:(p.sz||12),shape:(p.sh||'circle'),enabled:(p.en!==undefined?p.en:true),classes:{},count:0,shownCount:0};
...
// Live data
function connLive(){
  var sid=(el('live-sid').value||'').trim();if(!sid){lAlert('Enter Sheet ID','warn');return;}
  lAlert('Connecting...','info');
  fetch('https://docs.google.com/spreadsheets/d/'+sid+'/gviz/tq?tqx=out:csv')
    .then(function(r){return r.text();})
    .then(function(csv){
      LIVE={};csv.split('\n').slice(1).forEach(function(row){var c=row.split(',').map(function(v){return v.replace(/"/g,'').trim();});if(!c[0])return;if(!LIVE[c[0]])LIVE[c[0]]={pregnant:0,newborn:0,anc:0,highRisk:0};if(c[3]==='Pregnant_Woman')LIVE[c[0]].pregnant+=Number(c[4])||0;if(c[3]==='Newborn')LIVE[c[0]].newborn+=Number(c[4])||0;if(c[3]==='ANC_Visit')LIVE[c[0]].anc+=Number(c[4])||0;if(c[3]==='High_Risk')LIVE[c[0]].highRisk+=Number(c[4])||0;});
      lAlert('Connected! '+Object.keys(LIVE).length+' blocks.','ok');
      el('updt').textContent='Live: '+new Date().toLocaleTimeString('en-IN');
      if(CM==='live')applyBS();if(CURBLK)showBInfo(CURBLK);
    }).catch(function(){lAlert('Failed. Check Sheet ID.','err');});
}
function lAlert(msg,type){var e=el('live-alert');e.className='alert show '+type;e.textContent=msg;}
