...
var rvLyr=null,rvLblMks=[],rvLabelsOn=false;
var rvBlockOn={};  // per-block village visibility
var CURVILLAGE=null;  // currently selected village
...
var blockOn={};BNAMES.forEach(function(b){blockOn[b]=true;rvBlockOn[b]=true;});
...
window.onload=function(){
  ...
  map.on('zoomend moveend',function(){
    ...
    autoOpacity();
    if(el('chk-vl')&&el('chk-vl').checked)buildVillageLabels();
  });
  map.on('click',function(){
    if(CURBLK){
      ...
      showPH();
      renderExtFac();
      // Restore all village layers
      if(rvVisible){
        BNAMES.forEach(function(b){
          if(gpLyrs[b]&&gpOn[b]&&blockOn[b]!==false&&!map.hasLayer(gpLyrs[b]))
            gpLyrs[b].addTo(map);
        });
        blockLyr.bringToFront();
        if(extFacLyr)extFacLyr.bringToFront();
        if(el('chk-vl')&&el('chk-vl').checked)buildVillageLabels();
      }
    }
  });
    ...
    onEachFeature:function(f,lyr){
      ...
      lyr.on({
        ...
        mouseover:function(e){
          ...
          showTip('<b>'+nm+' Block</b><br><small>'+ns+' facilities</small>',e.originalEvent);
        },
...
function selBlock(nm,lyr){
  ...
  renderExtFac();
  // Village layer: show only selected block's villages
  if(rvVisible){
    BNAMES.forEach(function(b){
      if(!gpLyrs[b])return;
      if(b===nm){
        // Show this block's villages
        if(!map.hasLayer(gpLyrs[b]))gpLyrs[b].addTo(map);
      } else {
        // Hide all other blocks' villages
        if(map.hasLayer(gpLyrs[b]))map.removeLayer(gpLyrs[b]);
      }
    });
    blockLyr.bringToFront();
    if(extFacLyr)extFacLyr.bringToFront();
...
function togBlockLayer(on){
  ...
  togBlbl(on);
  // Redraw RV overlay if active (block toggle can shift SVG)
  var rvChk=el('chk-rv');
  if(rvChk&&rvChk.checked){
    setTimeout(function(){renderRVLayer();},100);
  }
}
...
// ===== REVENUE VILLAGE LAYER FUNCTIONS =====

// ===== VILLAGE (GP-STYLE) LAYER — Varanasi pattern =====
var gpLyrs={};     // per-block village L.geoJSON layers
var gpOn={};       // per-block visibility state
var gplblMks=[];   // village label markers
var rvVisible=false;
BNAMES.forEach(function(b){gpOn[b]=false;});

// Count villages per block
var VIL_COUNT={};
BNAMES.forEach(function(b){VIL_COUNT[b]=0;});
if(typeof RV_DATA!=='undefined'){
  RV_DATA.features.forEach(function(f){
    var b=f.properties.b||'';
    if(b)VIL_COUNT[b]=(VIL_COUNT[b]||0)+1;
  });
}

function togRVAll(on){
  rvVisible=on;
  var legVB=el('leg-vb');
  if(on){
    // Build per-block layers if not built yet
    buildGPLayers();
    // Show all blocks that are gpOn
    BNAMES.forEach(function(b){
      if(gpOn[b]&&blockOn[b]!==false){
        if(!map.hasLayer(gpLyrs[b]))gpLyrs[b].addTo(map);
      }
    });
    blockLyr.bringToFront();
    if(extFacLyr)extFacLyr.bringToFront();
    if(legVB)legVB.style.display='flex';
    rebuildVillageTogList();
  } else {
    BNAMES.forEach(function(b){
      if(gpLyrs[b]&&map.hasLayer(gpLyrs[b]))map.removeLayer(gpLyrs[b]);
    });
    togVillageLabels(false);
    var chk=el('chk-vl');if(chk)chk.checked=false;
    if(legVB)legVB.style.display='none';
...
function buildGPLayers(){
  if(typeof RV_DATA==='undefined')return;
  BNAMES.forEach(function(bnm){
    if(gpLyrs[bnm])return; // already built
    var feats=RV_DATA.features.filter(function(f){return f.properties.b===bnm;});
    if(!feats.length)return;
    var col=BCOLORS[bnm]||'#888';
    gpLyrs[bnm]=L.geoJSON({type:'FeatureCollection',features:feats},{
      style:function(){
        return{fillColor:col,fillOpacity:0.08,weight:0.8,color:'#555',opacity:0.7,dashArray:'4 3',stroke:true};
      },
      onEachFeature:function(f,l){
        var vname=f.properties.n||'';
        l.on({
          mouseover:function(e){
            l.setStyle({fillOpacity:0.35,weight:1.5,color:'#1a3c5e',dashArray:null});
            showTip('<b>'+vname+'</b><br><small>'+bnm+' Block</small>',e.originalEvent);
          },
          mouseout:function(){
            l.setStyle({fillColor:col,fillOpacity:0.08,weight:0.8,color:'#555',opacity:0.7,dashArray:'4 3'});
            hideTip();
          },
          click:function(e){
            L.DomEvent.stopPropagation(e);
            // Click hierarchy: block first, then village
            if(!CURBLK||CURBLK!==bnm){
              blockLyr.eachLayer(function(bl){
                if(bl.feature&&bl.feature.properties.tehs_name===bnm)selBlock(bnm,bl);
              });
              setTimeout(function(){showVillageDetail(vname,bnm);},500);
            } else {
              showVillageDetail(vname,bnm);
            }
          }
        });
      }
    });
    gpOn[bnm]=true; // default on
  });
}

function togGPForBlock(bnm,on){
  gpOn[bnm]=on;
  if(on){
    if(blockOn[bnm]!==false&&rvVisible){
      if(!gpLyrs[bnm])buildGPLayers();
      if(gpLyrs[bnm]&&!map.hasLayer(gpLyrs[bnm])){
        gpLyrs[bnm].addTo(map);
        blockLyr.bringToFront();
        if(extFacLyr)extFacLyr.bringToFront();
      }
    }
  } else {
    if(gpLyrs[bnm]&&map.hasLayer(gpLyrs[bnm]))map.removeLayer(gpLyrs[bnm]);
  }
  var row=el('brow-gp-'+bnm.replace(/\s/g,'_'));
...
function allGP(on){
  BNAMES.forEach(function(b){
    gpOn[b]=on;
    var chk=el('gpc-'+b.replace(/\s/g,'_'));
    if(chk)chk.checked=on;
    togGPForBlock(b,on);
  });
}

function rebuildVillageTogList(){
  var cont=el('gp-tog-list');if(!cont)return;
  cont.innerHTML='';
  BNAMES.forEach(function(nm){
    var col=BCOLORS[nm]||'#888';
    var cnt=VIL_COUNT[nm]||0;
    var div=document.createElement('div');
    div.className='brow on';div.id='brow-gp-'+nm.replace(/\s/g,'_');
    var safe=nm.replace(/\s/g,'_');
    div.innerHTML=
      '<div style="display:flex;align-items:center;flex:1;gap:6px;">'+
        '<div class="bdot" style="background:'+col+'"></div>'+
        '<div class="bnm">'+nm+'</div>'+
        '<div class="bgpc">'+cnt+' vil</div>'+
      '</div>'+
      '<label class="sw" onclick="event.stopPropagation()">'+
        '<input type="checkbox" id="gpc-'+safe+'" checked data-nm="'+nm+'" onchange="togGPForBlock(this.dataset.nm,this.checked)"/>'+
        '<span class="sl"></span>'+
      '</label>';
    cont.appendChild(div);
  });
...
// ===== VILLAGE LABELS =====
function togVillageLabels(on){
  gplblMks.forEach(function(m){map.hasLayer(m)&&map.removeLayer(m);});
  gplblMks=[];
  if(on&&rvVisible)buildVillageLabels();
}

function buildVillageLabels(){
  gplblMks.forEach(function(m){map.hasLayer(m)&&map.removeLayer(m);});
  gplblMks=[];
  if(typeof RV_DATA==='undefined')return;
  var z=map.getZoom();
  var size=z>=14?9:z>=12?8:7;
  var bounds=map.getBounds();
  RV_DATA.features.forEach(function(f){
    var blk=f.properties.b||'';
    if(!gpOn[blk])return;
    if(CURBLK&&blk.toLowerCase()!==CURBLK.toLowerCase())return;
    var coords=f.geometry.coordinates[0];
    if(!coords||!coords.length)return;
    var cx=0,cy=0;
    coords.forEach(function(c){cx+=c[0];cy+=c[1];});
    cx/=coords.length;cy/=coords.length;
    if(!bounds.contains([cy,cx]))return;
    var nm=f.properties.n||'';
    var ic=L.divIcon({
      className:'',
      html:'<div style="font-size:'+size+'px;font-weight:700;color:#37474f;text-shadow:1px 1px 0 #fff,-1px -1px 0 #fff,1px -1px 0 #fff,-1px 1px 0 #fff,0 0 3px #fff;white-space:nowrap;pointer-events:none;">'+nm+'</div>',
      iconSize:null,iconAnchor:null
    });
    var mk=L.marker([cy,cx],{icon:ic,interactive:false,zIndexOffset:-200});
    mk.addTo(map);gplblMks.push(mk);
  });
}

function showVillageDetail(vname,block){
  goTab('info',null);
  el('info-ph').style.display='none';
  el('info-det').style.display='block';
  el('iname').textContent=vname;
  el('isub').textContent=block+' Block • Barabanki District';
  el('istats').innerHTML='<div class="sbox"><div class="sv" style="color:#37474f;">'+block+'</div><div class="sl2">Block</div></div>';
  var ifl=el('ifacs');ifl.innerHTML='';
  if(FAC_RAW&&FAC_RAW.length>0){
    var vf=FAC_RAW.filter(function(f){
      return (f.village||f.name||'').toLowerCase().indexOf(vname.toLowerCase())>=0;
    });
    if(!vf.length){
      ifl.innerHTML='<div style="font-size:10px;color:var(--t3);padding:6px 0;">No facilities mapped to this village</div>';
    } else {
      vf.forEach(function(f){
        var col={DH:'#2e7d32',CHC:'#e53935',ACHC:'#e53935',PHC:'#1976d2',SC:'#f57c00'}[f.type]||'#888';
        var d=document.createElement('div');d.className='fi';
        d.innerHTML='<div class="fd" style="background:'+col+'"></div><div class="fn">'+f.name+'</div><div class="ft2">'+f.type+'</div>';
        ifl.appendChild(d);
      });
    }
  } else {
    ifl.innerHTML='<div style="font-size:10px;color:var(--t3);padding:6px 0;">Load facilities from sheet to see village data</div>';
  }
  el('isc-sec').style.display='none';
  if(window.innerWidth<=768)openSidebar();
}

// Legacy functions for compatibility
function togRV(on){togRVAll(on);}
function renderRVLayer(){}
function removeRVOverlay(){
  var old=document.getElementById('rv-svg-overlay');
  if(old)old.parentNode.removeChild(old);
  if(map._rvRedraw){map.off('move zoom moveend zoomend',map._rvRedraw);map._rvRedraw=null;}
}
